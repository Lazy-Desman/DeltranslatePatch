...
if (timer >= 9)
{
    ...
    for (var i = 0; i < sprite_get_number(sprite_index) / 2; i++) {
        xoffset[i] -= letterspeed1 / (i + 1)
    }
    for (var i = sprite_get_number(sprite_index) / 2 + 1; i < sprite_get_number(sprite_index); i++) {
        xoffset[i] += letterspeed1 / (sprite_get_number(sprite_index) - i)
    }
