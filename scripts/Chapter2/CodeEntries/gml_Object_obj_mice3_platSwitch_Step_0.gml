    ...
    if (toggle == 0)
    {
        ...
        for (i = 0; i < string_length(stringsetloc("ILOVEMOUSE", "obj_controller_city_mice3_slash_Draw_0_gml_18_0")); i++)
        {
            if (obj_controller_city_mice3.micescore == (i + 1)) {
                snd_play(scr_84_get_sound("snd_speak_and_spell_" + scr_letter_fix(string_char_at(stringsetloc("ILOVEMOUSE", "obj_controller_city_mice3_slash_Draw_0_gml_18_0"), (i + 1)))))
            }
        }
    }
    toggle = 1
